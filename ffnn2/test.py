...
class Client:
    ...
    def get_feature(self):
        '''return the feature vector (or tuple) of this user.'''
...
if __name__ == "__main__":
    ...
    num_users = 40000    # number of users to visit the website

    ## setup training arrangement
    episode_len = 1000        # the length of an episode for each training
    exploration_round = 40000  # explore so many rounds first
    ...
    for round in range(1,num_users+1):
        ...
        ## ..by exploitation
            ...
            if tuple(this_feature) in prediction_cache: # cache hit?
                reward_list = prediction_cache[tuple(this_feature)]
            else:
                #reward_list = model.predict(np.array([[this_feature]]), verbose=0) ###===
                reward_list = model.predict(np.array([this_feature]), verbose=0) ###===
                prediction_cache[tuple(this_feature)] = reward_list
            max_arm_index = np.argmax(reward_list, axis=1)  # shape: (1,)
        ...
        if len(memory_data)==episode_len:
            ...
            ##     ...                      ...
            ##  (agek,'cars',cark)]     (agek,[toy_pred,cark,sport_pred,food_pred])]
            ##
            ...
            #X = np.array([[d[0]] for d in [item[0] for item in memory_data]], dtype=np.float32) ###===
            ...
            ## feature (xi) to each prediction (yi_pred) for each access
            ...
            for this_feature, this_ad, this_reward in memory_data:
                this_reward_list = prediction[tuple(this_feature)]
                this_reward_list[Ad.AllArms.index(this_ad)] = this_reward
                train_data.append((this_feature, this_reward_list))

            ## prepare data from the train_data
            #X = np.array([[d[0]] for d in train_data], dtype=np.float32)      # shape: (N, 1) ###===
            X = np.array([list(d[0]) for d in train_data], dtype=np.float32) ###===
            y = np.array([d[1] for d in train_data], dtype=np.float32)        # shape: (N, 4)
    ...
    for arm in Ad.AllArms:
        ...
        for age in range(start, stop+1):
            feature_list.append(Client(age).get_feature())
        feature_list = np.array(feature_list, dtype=np.float32) 
        predictions = model.predict(feature_list, verbose=0)  # shape: (N, 4)

        #ages = np.array([[age] for age in range(start,stop+1)], dtype=np.float32)
        ###ages = np.array([[age, age] for age in range(start,stop+1)], dtype=np.float32)  # shape: (N,2)
        #predictions = model.predict(ages, verbose=0)  # shape: (N, 4)
        ad_predictions = predictions[:, Ad.AllArms.index(arm)]  # shape: (N,)
        age_score_dict = {
    ...
    for age in range(start, stop+1):
        ...
        #reward_list = model.predict(np.array([[age]]), verbose=0) ###===
